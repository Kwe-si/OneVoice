...
function listing($conn){
    $rep_details=mysqli_query($conn, "SELECT concat(P.fname,' ', P.lname) as Fullname, Ev.eventName, EvM.eventrole, D.deptName,
    case
    ...
    from Rep as R Inner Join Person as P on R.repID=P.personID
    Inner Join EventMember as EvM on EvM.personID=R.repID
...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-dark navbarcolor">
        ...
        <a href="reps.php"><button type="button" class="btn back-button">Back</button></a> 
            ...
            <?php foreach($data as $q){ ?>
                ...
                <td>
                        <form action="" method="post" onSubmit="onDelete(event)">
                            ...
                            <input type="text" name="ID" value="<?php echo $q["repID"] ?>" hidden/>  
                        </form>
                    </td>
                </tr>
