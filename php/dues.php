...
function delete($theID, $conn){
    // delete from dues in the database 
      $del="delete from `dues` where `duesID`='$theID'";
      $q=mysqli_query($conn,$del); 
}

//function to select and display dues status of members
$data=null;
function listing($conn){
    $dues = mysqli_query($conn,"SELECT d.duesID, P.personID, concat(P.fname, ' ', P.middlename, ' ', P.lname) as Fullname, DM.totalamount,
    if (DM.totalamount>D.amountpaid, concat(P.fname, ' paid Ghs', D.amountpaid, ' on ', monthname(DM.paymentdate), ' ', day(DM.paymentdate), ' ', 
    'via ', DM.modeofPayment, ' at ', time(DM.paymentdate), '.', ' Amount left to pay is Ghs', DM.totalamount-D.amountpaid),
    concat(P.fname, ' paid Ghs', D.amountpaid, ' on ', monthname(DM.paymentdate), ' ', day(DM.paymentdate), ' ', 
    'via ', DM.modeofPayment, ' at ', time(DM.paymentdate))) as Dues
    from Dues as D Inner Join DuesMember as DM on D.duesID = DM.duesID
    Inner Join Person as P on P.personID = D.duesID");
    $data=mysqli_fetch_all($dues,MYSQLI_ASSOC);
    return $data;
}
        ...
        <table class="table table-hover table-color table-bordered att">
            ...
            <?php foreach($data as $q){ ?>
                ...
                <?php if (in_array($q['Fullname'],$searchresult)) { ?>
                    ...
                    <td class="highlight"><?php echo $q['totalamount'] ?></td>
                    <td class="highlight"><?php echo $q['Dues'] ?></td>
                <?php } else { ?>
                        <td><?php echo $q['Fullname']?></td>
                        <td><?php echo $q['totalamount'] ?></td>
                        <td><?php echo $q['Dues']?></td>
                <?php } ?>
                <td>
                        <form action="" method="post" onSubmit="onDelete(event)">
                            <input class="btn btn-danger" type = "submit" name="Delete"  value="Delete"/>
                            <input type="text" name="ID" value="<?php echo $q["duesID"] ?>" hidden/>  
                        </form>
                </td>
                </tr>
            <?php } ?>
